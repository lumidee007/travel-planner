export async function handleSubmit(event) {
  ...
  if (date !== "" && destination !== "" && $Date > $currentDate) {
    let daysDiff = Math.ceil(
      (new Date(date) - new Date()) / (1000 * 3600 * 24)
    );

    try {
      // PIXABAY FETCH REQUEST
      let API_KEY_PIXABAY = "18393364-b93a8cbe009d33fa4364578e1";
      let pixabay_res = await fetch(
        `https://pixabay.com/api/?key=${API_KEY_PIXABAY}&q=${destination}&image_type=photo`
      );
      let pixabay_data = await pixabay_res.json();
      let destinationImg =
        pixabay_data.hits.length > 0
          ? pixabay_data.hits[0].webformatURL
          : "https://images.unsplash.com/photo-1495954484750-af469f2f9be5?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=MnwxNDI0NzB8MHwxfHJhbmRvbXx8fHx8fHx8fDE2MzI1NTQ0MjY&ixlib=rb-1.2.1&q=80&w=1080";

      //using Openweather API
      let API_KEY_OPENWEATHER = "ae8183305930109bde697b966d0122af";
      const open_res = await fetch(
        `https://api.openweathermap.org/data/2.5/weather?q=${destination}&appid=${API_KEY_OPENWEATHER}`
      );
      ...
      let API_KEY_WEATHERBIT = "07782d4256f14f2289661151ca6f3aa8";
      let API_KEY_WEATHERBIT = "af46911e07bd4dd499726fce845f3e09";
      let weather_res =
        daysDiff > 7
          ? await fetch(
              `https://api.weatherbit.io/v2.0/forecast/daily?city=${destination}&key=${API_KEY_WEATHERBIT}`
            )
          : await fetch(
              `https://api.weatherbit.io/v2.0/current?lat=${lat}&lon=${long}&key=${API_KEY_WEATHERBIT}&include=minutely`
            );

      const weather_data = await weather_res.json();
      let maxTemp =
        weather_data.data[0].max_temp !== undefined
          ? weather_data.data[0].max_temp
          : "--";
      let minTemp =
        weather_data.data[0].min_temp !== undefined
          ? weather_data.data[0].min_temp
          : "--";
      ...
      const result = await db_response.json();
    } catch (error) {
      console.log(error);
    }
  } else {
    alert("Please ensure the destination and travel date is correct");
  }
}
